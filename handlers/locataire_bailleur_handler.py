...
# === Gestion des étapes de locataire ===
...
def handle_add_tenant_landlord(update: Update, context: CallbackContext):
    ...
    add_tenant(
        context.user_data.get('tenant_name'),
        context.user_data.get('tenant_email'),
        context.user_data.get('tenant_address'),
        context.user_data.get('tenant_rent'),
        context.user_data.get('tenant_frequency'),
        context.user_data.get('tenant_landlord')
    )
...
def handle_add_landlord_address(update: Update, context: CallbackContext):
    add_landlord(
        context.user_data.get('landlord_name'),
        update.message.text
    )
